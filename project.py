...
from .addon import DetectedCgateAddon, detect_cgate_addons
from .client import (
    CgateEndpoint,
    CgateError,
    ...
    return vol.Schema(
        {
            ...
            vol.Required(CONF_PROJECT_NAME, default=project_name): TextSelector(
                TextSelectorConfig(type=TextSelectorType.TEXT)
            ),
        }
    )
...
class CbusCgateConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    def _detect_addons(self, *, refresh: bool = False) -> list[DetectedCgateAddon]:
        """Return running companion add-ons without requiring Supervisor."""
        if self._detected_addons is not None and not refresh:
            return self._detected_addons
        try:
            self._detected_addons = detect_cgate_addons(get_addons_info(self.hass))
        except (HassioNotReadyError, KeyError):
            self._detected_addons = []
        return self._detected_addons

    def _addon_by_slug(self, slug: str) -> DetectedCgateAddon | None:
        return next(
            (addon for addon in self._detect_addons(refresh=True) if addon.slug == slug),
            None,
        )
    ...
    async def async_step_user(
        ...
        if self._detect_addons(refresh=True):
    ...
    async def async_step_addon_project(
        ...
        addons = self._detect_addons(refresh=True)
        if not addons:
            return self.async_abort(reason="addon_not_found")

        errors: dict[str, str] = {}
        if user_input is not None:
            addon = self._addon_by_slug(str(user_input[CONF_ADDON_SLUG]))
            if addon is None:
                ...
                if not project_name:
                    ...
                    try:
                        self._project = await self._async_fetch_project(
                            project_name,
                            self._connection,
                            f"{project_name}.xml (fetched from {addon.name} add-on)",
                        )
                    except CgateError as err:
                        _LOGGER.warning("Unable to fetch Toolkit project from add-on: %s", err)
